...
const assets={};
function collect(dir,prefix=''){for(const name of fs.readdirSync(dir)){const p=path.join(dir,name),key=prefix+'/'+name;if(fs.statSync(p).isDirectory())collect(p,key);else assets[key]=fs.readFileSync(p).toString('base64');}}
collect('public');
